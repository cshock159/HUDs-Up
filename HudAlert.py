...
setting_image = str(configParser.get('settings','image'))
...
#### Add Profile Edit 
#### Add in proper text coloring

start_stop = [
    [sg.Text("Start/Stop",font=("Comic Sans MS","18","bold"))],
...
audio_autoclose = [
    [sg.Text("Options",font=("Comic Sans MS","18","bold"))],
    ...
    [sg.Text("Allows you to choose how sensitive HUDs-Up is. Higher = Less Sensitive.",font=("Comic Sans MS","9"))],
    [sg.Slider(range=(0,1),orientation='h',tick_interval=.5,resolution=.05,default_value=setting_confidence,key="confidence",tooltip="Sets confidence for image detection. Higher = Less Sensitive.")],
    [sg.Text(("Allows you to select how often, in seconds, Huds-Up checks."),font=("Comic Sans MS","9"))],
    [sg.Slider(range=(0,60),orientation='h',tick_interval=15,resolution=1,default_value=setting_frequency,key="frequency",tooltip="Determines the frequency in which HUDs-Up checks for the HUD.")],
    [sg.Checkbox(text="Audio Chime",default=setting_chime,tooltip="Plays an audio chime when HUDs-Up detects the HUD.",key="audio_cb"),
    sg.Checkbox(text="Auto Close HUD",default=setting_auto_close,tooltip="Auto Closes the HUD when detected.",key="autoclose_cb"),
...
def keyPush():
    ...
    while True:
        image_location = pyautogui.locateOnScreen(image_file,confidence=values["confidence"])
...
def findmyimage():
    if values["uploaded_image"]:
        image_file = str(values["uploaded_image"])
    elif setting_image:
        image_file = setting_image
    elif setting_image == "" :
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        try:
            ...
            configParser['settings']['image'] = image_file
